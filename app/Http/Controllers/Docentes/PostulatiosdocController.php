...

{
    ...
    */
    ...
    public function verposactdoc()
    {
        ...
        ->where('con.formulario', "proceso1")

        ->get(['postulations.status', 'postulations.idpost', 'con.statuspos', 'con.title', 'u.name', 'u.surname', 'cat.namecat as namecat']);
        ...
        foreach($post as $po)
        {
            ...
            array_push($array1, array('status' => $po->status, 'statusconcurso' => $po->statuspos, 'title' => $po->title, 'name' => $po->name.' '.$po->surname, 'namecat' => $po->namecat, 'answ' => $array2 ) );
        }
        ...
        if($post == 0)
        {
            switch (true) 
            {
                ...
                case ($form == "proceso2"):
                    ...
                    $ar = DB::table('etapa1')->insertGetId(['id_post' => $idpost]);

                    $ar1 = DB::table('etapa2')->insertGetId(['id_post' => $idpost]);

                    $ar2 = DB::table('etapa3')->insertGetId(['id_post' => $idpost]);
                    ...
                    {
                        DB::table('etapa1')->where(['id' => $ar])->delete();
