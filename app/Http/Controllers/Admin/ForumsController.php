...

{
    ...
    */
    ...
    public function ingforpubadm(Request $request)
    {
        switch (true) 
        {
            case ($request->type == "add"):
                ...
                switch (true) 
                {
                    case ($name == 0):
                        ...
                        if($add)
                        {
                            return redirect()->route('acciones.foro.publico.administrador')->with('success', "El foro ha sido creado correctamente.");
                        }
                        else
                        {
                            return redirect()->route('acciones.foro.publico.administrador')->with('danger', "El foro no ha sido creado, Inténtelo nuevamente.");
                        }
                        
                    break;

                    case ($name == 1):
                        return redirect()->route('acciones.foro.publico.administrador')->with('info', "El nombre del foro ya existe.");
                    break;
                    
                    default:
                        return redirect()->route('acciones.foro.publico.administrador')->with('danger', "Hay un error al ingresar el nombre del foro.");
                    break;
                }
            break;
            case ($request->type == "edi"):
                ...
                switch (true) 
                {
                    case ($name == 0):
                        ...
                        if($add)
                        {
                            return redirect()->route('acciones.foro.publico.administrador')->with('success', "El nombre de foro ha sido editado correctamente.");
                        }
                        else
                        {
                            return redirect()->route('acciones.foro.publico.administrador')->with('danger', "El foro no ha sido editado, Inténtelo nuevamente.");
                        }
                    ...
                    default:
                        return redirect()->route('acciones.foro.publico.administrador')->with('danger', "Hay un error al editar el nombre del foro.");
                    break;
                }
            break;
            case ($request->type == "del"):
                ...
                $add = DB::table('forum_public')->where('id', $id)->update(["statusforum" => 0, 'updated_at' => date('Y-m-d H:i:s')]);
                        if($add)
                        {
                            return redirect()->route('acciones.foro.publico.administrador')->with('success', "El foro ha sido eliminado correctamente.");
                        }
                        else
                        {
                            return redirect()->route('acciones.foro.publico.administrador')->with('danger', "El foro no ha sido eliminado, Inténtelo nuevamente.");
                        }
            break;
    ...
    public function lisusuestingforpub($idcat = null, $tipo = null)
    {
    ...
    
    {
        ...
        if($cat > 0)
        {
            ...
            return view('forums.ingresoadmin', compact('cat'), ['usact' => $us, 'visitas' => $visitas, 'votos' => $votos, 'respuestas' => $respuestas, 'themes' => $the] );
        }
        else
        {
    ...
    public function addnuetemforpubadm(Request $request)
    {
        ...
        if($part > 0)
        {
            ...
            if($val > 0)
            {
                ...
                return redirect()->route('acceder.forum.usuarios.activos.foro.publico', ["idcat" => $request->idcat])->with('success', trans('multi-leng.formerror139'));
            }
            else
            {
                return redirect()->route('acceder.forum.usuarios.activos.foro.publico', ["idcat" => $request->idcat])->with('warnig', trans('multi-leng.formerror140'));
