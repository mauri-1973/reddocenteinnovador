...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#dt-mant-table').DataTable({
            ...
            @role('admin')
                ...
                ajax: "{{ route('acciones.foro.publico.administrador') }}",

            @endrole
            @hasanyrole('docente|user')

                ajax: "{{ route('ver.contenido.foro.publico.usuario.registrado') }}",
            ...
            @endhasanyrole
                ...
                { data: 'idencrypt', name: 'idencrypt', render: function ( data, type, row ) {
                    ...
                    return `<a href="{{ url('acceder-forum-usuarios-activos-foro-publico') }}/${data}" class="btn btn-dark btn-sm btn-block mb-1">{{ trans('multi-leng.formerror120') }}</a>
                    ...
                    return `<a href="{{ url('acceder-forum-publico-usuarios-activos') }}/${data}" class="btn btn-dark btn-sm btn-block mb-1">{{ trans('multi-leng.a298') }}</a>`;
            ...
            @hasanyrole('docente|user')
                ...
                columnDefs: [
                {
                        targets: [3, 4, 5], // Índices de columnas a ocultar (0 = primera, 3 = cuarta, etc)
                        visible: false,
                        searchable: false // Opcionalmente también evitar que se busque en esa columna
                    }
                ],
    ...
    function editcat(nombre, id)
    {
        ...
        $( "#modalbody" ).html(`<form id="formadd" method="POST" action="{{ route('ingreso.foro.publico.administrador') }}" enctype="multipart/form-data">
        @csrf
    ...
    function searchus(idcar, tipo)
    {
        ...
        $.ajax({
            ...
            url: '{{route("ingreso.foro.publico.administrador")}}',
            ...
            success: function(data) {
                ...
                if(data.tipo == 0)
                {
                    ...
                    {
                        window.location.href = '{{ url("listado-usuarios-estado-ingreso-foro-publico") }}/'+idcar+'/'+tipo;
                    }
                }
                if(data.tipo == 1)
                {
