             ...
             <div class="card">
                <div class="card-body">
                    @role('blog')
                        ...
                        <a class="btn btn-success" href="{{route('acciones.foro.publico.blog')}}" role="button">{{ trans('lang.volver')}}</a>
...
<form name="delete-forum" id="delete-forum" method="POST" action="{{ route('busquedas.info.forums.publicos.blog') }}" enctype="multipart/form-data">
@csrf
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $('#dt-mant-table').DataTable({
                ...
                fixedHeader: true,
                ajax: {
                    url: "{{ route('acceder.forum.usuarios.activos.foro.publico.ajax.blog') }}",
                    data: function(d) {
                ...
                columns: [
                    ...
                    { data: 'votos', render: function ( data, type, row ) {
                            var html = '';
                            if(row.iduser == row.idusuario)
                            {
                                html += `<button type="button" class="btn btn-warning btn-sm float-right mr-1" onclick="editarforum('${row.encrypt}')">{{ trans('lang.editar')}}</button>
                            ...
                            return `<div class="card row-hover pos-relative py-3 px-3 mb-3 border-warning border-top-0 border-right-0 border-bottom-0 rounded-0">
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-12 mt-2">
                                            ...
                                            <a class="btn btn-success btn-sm float-right mr-1" href="{{ url('ver-contenido-tema-foro-publico-blog' ) }}/${row.encrypt}" role="button" style="width:200px;" data-toggle="tooltip" data-placement="top" title="Haga click para ver el contenido completo del tema ingresado por el usuario" data-html="true">{{ trans('multi-leng.formerror70')}}</a>
                                            ${html}
                                        </div>
                                    </div>
                                </div>`;
    ...
    function editarforum(idcript)
    {
        ...
        url: '{{ route("busquedas.info.forums.publicos.blog") }}',
        ...
        success: function(data) {
            ...
            $( "#modalbodyforo" ).html(`<form class="formeditsub" id="formeditsub" method="POST" action="{{ route('busquedas.info.forums.publicos.blog') }}" enctype="multipart/form-data">
                @csrf
    ...
    $(".addtopic").click(function(e){
        ...
        $( "#modalbodyforo" ).html(`<form class="formaddsub" id="formaddsub" method="POST" action="{{ route('agregar.nuevo.tema.forum.publico.admin.blog') }}" enctype="multipart/form-data">
            @csrf
    ...
    $(".viewus").click(function(e){
        ...
        url: '{{route("buscar-informacion-usuario-contacto-blog")}}',
