                    ...
                    <table id="dt-mant-table" class="table table-bordered display responsive nowrap" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @if($tipo == 0)
                                <td>
                                    <form id="formadd{{ $array[$row]['idforpar'] }}" method="POST" action="{{ route('ingreso.foro.publico.administrador') }}" enctype="multipart/form-data">
                                    @csrf
                                ...
                                @if($tipo == 1)
                                <td>
                                    <form id="formadd{{ $array[$row]['idforpar'] }}{{ $array[$row]['idforpar'] }}" method="POST" action="{{ route('ingreso.foro.publico.administrador') }}" enctype="multipart/form-data">
                                    @csrf
                                    ...
                                    <form id="formadd{{ $array[$row]['idforpar'] }}{{ $array[$row]['idforpar'] }}{{ $array[$row]['idforpar'] }}" method="POST" action="{{ route('ingreso.foro.publico.administrador') }}" enctype="multipart/form-data">
                                    @csrf
                                        <input type="hidden" name="tipo" name="tipo" value="3">
                                        <input type="hidden" id="idfor" name="idfor" value="{{ Crypt::encrypt($array[$row]['idforpar'] )}}">
                                        <button type="submit" class="btn btn-danger btn-sm btn-block mt-1">{{ trans('multi-leng.formerror186')}}</button>
                                ...
                                @if($tipo == 2)
                                <td>
                                    <form id="formadd{{ $array[$row]['idforpar'] }}{{ $array[$row]['idforpar'] }}{{ $array[$row]['idforpar'] }}{{ $array[$row]['idforpar'] }}" method="POST" action="{{ route('ingreso.foro.publico.administrador') }}" enctype="multipart/form-data">
                                    @csrf
                                        <input type="hidden" name="tipo" name="tipo" value="4">
                                        <input type="hidden" id="idfor" name="idfor" value="{{ Crypt::encrypt($array[$row]['idforpar'] )}}">
                                        <button type="submit" class="btn btn-success btn-sm btn-block mt-1">{{ trans('multi-leng.formerror185')}}</button>
