...
<form  id="formpostnew" method="post" action="{{ route('finalizar-ingreso-concurso-administrador')}}" enctype="multipart/form-data" style="display:none">
    ...
    <input type="hidden" name="form" id="form" value="{{ $post->formulario }}">
...
@section('extra-script')
    ...
    <script>
        ...
        function actualizardatos(val, type)
        {
            ...
            formData.append("codificado", encodeURIComponent(val));
            
            const size = new TextEncoder().encode(JSON.stringify(formData)).length
            const kiloBytes = size / 1024;
            const megaBytes = kiloBytes / 1024;
            console.log('peso', megaBytes);
