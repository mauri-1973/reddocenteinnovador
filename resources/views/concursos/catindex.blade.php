                    ...
                    <table id="dt-mant-table" class="table table-bordered display responsive nowrap" style="width:100%">
                        ...
                        @foreach($categories as $category)
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-success btn-sm btn-block editcat mb-1" data-id='{{ $category->idcatcom }}' data-nombre='{{ $category->namecat }}'>{{ trans('multi-leng.editarcateg')}}</button>
                                    <button type="button" class="btn btn-danger btn-sm btn-block delcat" data-id='{{ $category->idcatcom }}' data-nombre='{{ $category->namecat  }}'>{{ trans('multi-leng.eliminarcateg')}}</button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
...
<script type="text/javascript">
    ...
    function validarnombre(nombre)
    {
        ...
        $.ajax({
            url: '{{route("validar-nombre-categoria-concurso-administrador")}}',
            type: 'POST',
            async: false,
            dataType: 'json',
            data: {name: nombre},
