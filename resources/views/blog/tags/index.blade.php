                    ...
                    <table id="dt-mant-table" class="table table-bordered display responsive nowrap" style="width:100%">
                        ...
                        @foreach($tags as $tag)
                            <tr>
                                ...
                                <td>
                                    <button type="button" class="btn btn-success btn-sm btn-block editcat mb-1" data-id='{{ $tag->id }}' data-nombre='{{ $tag->title }}'>{{ trans('multi-leng.formerror62')}}</button>
                                    <button type="button" class="btn btn-danger btn-sm btn-block delcat" data-id='{{ $tag->id }}' data-nombre='{{ $tag->title  }}'>{{ trans('multi-leng.formerror63')}}</button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
...
<script type="text/javascript">
    ...
    function validarnombre(nombre)
    {
        ...
        $.ajax({
            url: '{{route("validar-nombre-tag-blog")}}',
            type: 'POST',
            async: false,
            dataType: 'json',
            data: {name: nombre},
